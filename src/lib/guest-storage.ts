...
import { mapGuestToVisitor, mapVisitorToGuest, toFirestoreDate } from './mappers';
...
export class GuestStorage {
  ...
  static async checkDuplicateGuest(name: string, phone: string, excludeId?: string): Promise<boolean> {
    try {
      ...
      if (excludeId) {
        return !querySnapshot.docs.every((doc: DocumentData) => doc.id === excludeId);
      }
  ...
  static async cleanupExpiredTrash(): Promise<number> {
    try {
      ...
      const deletedVisitors = await getVisitors(true);
      const expiredVisitors = deletedVisitors.filter(visitor => 
        visitor.deletedAt && toFirestoreDate(visitor.deletedAt) <= thirtyDaysAgo
      );
      
      await Promise.all(expiredVisitors.map(visitor => 
        hardDeleteVisitor(visitor.id!)
      ));
    ...
    try {
      ...
      return {
        ...
        totalThisMonth: allVisitors.filter(v => {
          const visitDate = toFirestoreDate(v.checkIn);
          return visitDate.getMonth() === new Date().getMonth() && 
                 visitDate.getFullYear() === new Date().getFullYear();
        }).length,
        totalThisYear: allVisitors.filter(v => {
          return toFirestoreDate(v.checkIn).getFullYear() === new Date().getFullYear();
        }).length,
        currentlyCheckedIn: activeVisitors.filter(v => !v.checkOutTime).length,
