...
export function firestoreToGuest(firestoreGuest: FirestoreGuest & { id: string }): import('@/types/guest').Guest {
  return {
    ...
    checkOutTime: firestoreGuest.checkOutTime?.toDate().toLocaleTimeString('id-ID'),
...
export function guestToFirestore(guest: import('@/types/guest').Guest): Omit<FirestoreGuest, 'id'> {
  ...
  return {
    checkIn: guest.visitDate ? Timestamp.fromDate(guest.visitDate) : now,
    checkOutTime: guest.checkOutTime ? Timestamp.fromDate(new Date(`${guest.visitDate.toDateString()} ${guest.checkOutTime}`)) : null,
    ...
    deletedAt: guest.deletedAt ? Timestamp.fromDate(guest.deletedAt) : null,
    ...
    scheduledDate: guest.scheduledDate ? Timestamp.fromDate(guest.scheduledDate) : null,
