...
const DashboardPage = () => {
  ...
  // Removed unused state variables: allLastVisible, hasMoreAll, isLoadingMoreAll, setIsLoadingMoreAll
  ...
  const loadAllVisitors = useCallback(async () => {
    if (activeTab === 'all' && allVisitors.length === 0) {
      try {
        ...
        const { visitors: fetchedVisitors } = await getAllVisitors();
  ...
  // Removed unused _loadMoreAll function as it's not currently in use
  ...
  }, [activeTab, allPage, allVisitors, pageSize, visitors]);
  
  // For active tab, always show 1 page since we're showing all
  const totalActivePages = 1;
  
  // For all tab, calculate total pages based on total count and page size
